...
// 헤드, 테일이 2개씩 붙은 패킷으로 인코드
void BUG_encodePacket3(char* packetArr, PACKET pk)
{
	int offset = 0;
	pk.phead.bodysize + sizeof(HEADER);
	// HEADER 복사
	memcpy(packetArr, &pk, sizeof(HEADER)); 
	offset += sizeof(HEADER);
	memcpy(packetArr + offset, &pk, sizeof(HEADER)); 
	offset += sizeof(HEADER);
...
// 헤드와 테일만 있는 패킷
void BUG_encodePacket4(char* packetArr, PACKET pk)
{
	int offset = 0;

	pk.phead.bodysize = sizeof(HEADER) * 2;
	// HEADER 복사
	memcpy(packetArr, &pk, sizeof(HEADER)); 
	offset += sizeof(HEADER);
