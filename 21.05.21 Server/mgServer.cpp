...
void mgServer::mg_server_start()
{
	...
	usleep(100);
	m_banManager.start(1000000, 50); // 1초에 50번이상 접속시도하면 밴
	thread watcherThread(&userManager::is_alive_thread, &m_userManager);
	thread banThread(&banManager::banThreadWork, &m_banManager);
...
void mgServer::mg_connect()
{
	...
	client_fd = accept(m_server_fd, (struct sockaddr*) &client_addr, (socklen_t*) &client_len);

	// BAN
	/////////////////////////////////////////////////////////
	string client_IP(inet_ntoa(client_addr.sin_addr));

	m_banManager.add_IP(client_IP);

	if(m_banManager.is_Banned(client_IP) == true)
	{
		cout << "BANNED IP TRY TO CONNECT : " << client_IP << endl;
		close(client_fd);
		usleep(10);
	}
	////////////////////////////////////////////////////////
 ...
 void mgServer::mg_login(int client_fd)
{
	...
	tv.tv_sec = 30;
	tv.tv_usec = 0;
	setsockopt(client_fd, SOL_SOCKET, SO_RCVTIMEO, (const char*) &tv, sizeof(timeval));
	///////////////////////////////////////////////////////////////////
	...
	{
		cout << "Login FAIL! == Client Fd : " << client_fd << endl;
	...
	{
		cout << "Login Success! == Client Fd : " << client_fd << endl;
